...
// 1. Bordas restauradas para o peso original (mais visíveis)
function estiloPadrao(feature) {
    return {
        fillColor: '#70cbe9',
        weight: 1,            // Voltou para 1
        opacity: 1,           // Sem transparência na borda
        color: 'white',       
        fillOpacity: 1      
    };
}
...
function interacoesPorMunicipio(feature, layer) {
    ...
    // Etiqueta com o nome (ela é o "retângulo", mas agora a borda do município vai se destacar mais que ela)
    layer.bindTooltip(nomeMunicipio, {
        sticky: true,
        direction: 'auto'
    });

    // 2. Hover restaurado para o destaque forte no shape do município
    layer.on({
        mouseover: (e) => {
            const munLayer = e.target;
            munLayer.setStyle({ 
                weight: 3,       // Voltou para 3 (destaque forte no formato geográfico)
                color: '#333', 
                fillOpacity: 1 
            });
